...
def scroll_to_tag(view, tag, hook=None):
    ...
    def and_then(view):
        ...
        if do_find:
            symbol_region = view.find('^'+tag.ex_command, look_from, 0)
        ...
        if do_find and symbol_region:
            ...
            status_message('Can\'t find "%s"' % tag.ex_command)
...
class NavigateToDefinition(sublime_plugin.TextCommand):
    ...
    def run(self, view, args, tags_file):
        ...
        m = re.compile('\W(.*)?:$').search(line_to_symbol)
        if m is not None:
